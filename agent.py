...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        self.trial_counts = self.trial_counts + 1
    ...
    def choose_action(self, state):
        ...
        if self.learning == True:
            if self.epsilon > random.random():
                ...
                highest_q_actions = [key for key in self.Q[state].keys() if self.Q[state][key] == highest_q]
    ...
    def learn(self, state, action, reward):
        ...
        if self.learning == True:
            new_q = self.alpha * reward
            old_q = self.Q[state][action]
            self.Q[state][action] = (1 - self.alpha) * old_q + new_q
        return
...
def run():
    ...
    sim.run(tolerance=0.03, n_test=30)
